...
impl Header {
        ...
        R: Read,
    {
            ...
            *variant = Variant::GNU;
            if identifier == GNU_SYMBOL_LOOKUP_TABLE_ID {
                ...
                *name_table = vec![0; size as usize];
                reader.read_exact(name_table as &mut [u8])?;
                return Ok(Some((Header::new(identifier, size), header_len)));
            }
            let start =
                parse_number("GNU filename index", &buffer[1..16], 10)? as usize;
        ...
        let uid = parse_number("owner ID", &buffer[28..34], 10)? as u32;
        let gid = parse_number("group ID", &buffer[34..40], 10)? as u32;
    ...
    fn parse_symbol_table_if_necessary(&mut self) -> io::Result<()> {
        ...
        if let Some(ref header_and_loc) = self.symbol_table_header {
            ...
            if self.variant == Variant::GNU {
                let num_symbols = reader.read_u32::<BigEndian>()? as
                    usize;
                ...
                let num_symbols =
                    (reader.read_u32::<LittleEndian>()? / 8) as usize;
                let mut symbol_offsets =
                ...
                reader.read_exact(&mut str_table_data)?;
                let mut symbol_table = Vec::with_capacity(num_symbols);
